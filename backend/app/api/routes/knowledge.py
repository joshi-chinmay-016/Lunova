...
async def ingest_document(
    ...
    doc = await DocumentService.ingest_document(db, data, company_id)
    
    # Process the document directly in background
    background_tasks.add_task(DocumentService.process_document_bg, doc.id, company_id, data.content)
...
async def list_documents(
    ...
    return await DocumentService.get_documents(db, company_id)
